...
import (
	...
	"obyoy-backend/store/translation"
...
func (count *countByStatusReader) askStore(StatusID string) (
	...
	counts, err = count.translations.Count()
	return
}
...
func (count *countByStatusReader) Count(
	...
	if err != nil {
		message := "Could not count translation by status id error : "
		count.logError(message, err)
		err = count.giveError()
		return count.giveErrorResponse(err)
	}

	var resp dto.CountResp = count.prepareResopnse(counts)
